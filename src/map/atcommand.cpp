...
ACMD_FUNC(reload) {
	...
	if (strstr(command, "itemdb") || strncmp(message, "itemdb", 4) == 0) {
		...
		clif_displaymessage(fd, msg_txt(sd, 798)); // Item Synthesis database has been reloaded.
	} else if (strstr(command, "upgradedb") || strncmp(message, "upgradedb", 6) == 0) {
		item_upgrade_db_reload();
		clif_displaymessage(fd, msg_txt(sd, 799)); // Item Upgrade database has been reloaded.
	}
...
ACMD_FUNC( stylist ){
	...
	clif_displaymessage( fd, msg_txt( sd, 1600 ) ); // This command requires packet version 2015-11-04 or newer.
	return -1;
#else

	if( sd->state.stylist_open ){
		clif_displaymessage( fd, msg_txt( sd, 1601 ) ); // You have already opened the stylist UI.
		return -1;
	}
